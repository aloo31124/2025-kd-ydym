...
svgedit.utilities.getPathBBox = function(path) {
	...
	if(!seglist){
		...
		var p =path.getBoundingClientRect();
		return {
			'x': p.left*0.6,
			'y': p.top*0.7,
			'width': p.width,
...
//added by job 2019/1/9
//Function: svgedit.utilities.getStartPositionOfChar
//get the position of char at pos
//
//Parameters:
//elem - (text) DOM element to get Char
//pos - position of char 
svgedit.utilities.getStartPositionOfChar = function (textEl, pos) {


    if (svgedit.browser.isEdge() && textEl.textContent.length == 1 && textEl.textContent.codePointAt(0) == "12288") {

        //var endPos = textEl.getStartPositionOfChar(pos);

        //console.log("before :" + endPos.x + ":" + endPos.y);

        textEl.innerHTML = textEl.innerHTML.replace(String.fromCodePoint(12288), String.fromCodePoint(12288) + "{0}");

        var endPos = textEl.getStartPositionOfChar(pos);

        //console.log("after:" + endPos.x + ":" + endPos.y);

        textEl.innerHTML = textEl.innerHTML.replace(String.fromCodePoint(12288) + "{0}", String.fromCodePoint(12288));

        return endPos;


    } else {
        return textEl.getStartPositionOfChar(pos);
    }
...
//added by job 2019/1/9
//Function: svgedit.utilities.getEndPositionOfChar
//get the position of char at pos
//
//Parameters:
//elem - (text) DOM element to get Char
//pos - position of char 
svgedit.utilities.getEndPositionOfChar = function(textEl, pos) {

   
    if (svgedit.browser.isEdge() && textEl.textContent.length == 1 && textEl.textContent.codePointAt(0) == "12288") {

        //var endPos = textEl.getEndPositionOfChar(pos);
        ...
        return textEl.getEndPositionOfChar(pos);
    }
...
svgedit.utilities.getBBox = function(elem) {
	...
	switch ( elname ) {
        ...
        } else if (selected.textContent.codePointAt(0) == "12288" && selected.textContent.length == 1 && svgedit.browser.isEdge()) {
            //when textContent only contains character codepoint 12288, edge would return 0
            //replace character before get BBox
            //job 2019/1/9

            selected.innerHTML = selected.innerHTML.replace(String.fromCodePoint(12288), '{a}');
            ret = selected.getBBox();
            selected.innerHTML = selected.innerHTML.replace('{a}', String.fromCodePoint(12288));
...
svgedit.utilities.assignAttributes = function(node, attrs, suspendLength, unitCheck) {
	...
	for (var i in attrs) {
		...
		//if (node.id=='text_cursor' && attrs[i]=='visible'){
		//	console.log(node);
		//	console.log(i);
		//	console.log(attrs[i]);
		//}
