...
$.SvgCanvas = function (container, config) {
    ...
    var svgroot = svgdoc.importNode(svgedit.utilities.text2xml(
        '<svg id="svgroot' + curConfig.backId + '" xmlns="' + svgns + '" xlinkns="' + xlinkns + '" ' +
    ...
    var getVisibleElements = this.getVisibleElements = function (parent) {
        ...
        if (parent.find("#canvas_background" + curConfig.backId).length) parent.splice(0, 1) // Prevent background from being included
        var contentElems = [];
    ...
    (function () {
        ...
        var mouseDown = function (evt) {
            ...
            switch (current_mode) {
                ...
                case "resize":
                    ...
                    init_bbox = svgedit.utilities.getBBox($('#selectedBox0' + curConfig.backId)[0]);
                    var bb = {};
                ...
                case "rotate":
                    ...
                    document.getElementById('workarea' + curConfig.backId).className = "rotate";
            ...
            if (current_mode) {
                document.getElementById('workarea' + curConfig.backId).className = (current_mode == "resize") ? evt.target.style.cursor : current_mode
            }
        };
        ...
        var mouseUp = function (evt) {
            ...
            if (!keep && element != null) {
                ...
                if ((current_mode != "path" || !drawn_path) &&
                    ...
                    t.id != ("svgcanvas" + curConfig.backId) && t.id != ("svgroot" + curConfig.backId)) {
        ...
        var dblClick = function (evt) {
            ...
            if (tagName === 'svg') {
                ...
                var current_layer = getCurrentDrawing().getCurrentLayer();
                var contexts = current_layer.querySelectorAll('text[id*="KDRichTextBlock"][id*="_context"]');
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    if (lenTspan > 0) {
                        for (var j = 0; j < chars; j++) {
                            ...
                            end = svgedit.utilities.getEndPositionOfChar(curtext, j);
                            //console.log(Ext.clone(end));
                            //console.log(Ext.clone(end));
                            //var isTurn = tpans[j].hasAttribute('dy');
                            //if ((j + 1) == lenTspan) isTurn = true;
                            if (ptCurrent.x < end.x && ptCurrent.y < end.y && ptCurrent.y >= topLimit) {
                                var isReSelect = current_mode !== 'textedit';

                                if (isReSelect) {
                                    textActions.select(curtext, end.x * current_zoom, end.y * current_zoom);
                                    textActions.setCursor(j);
                                    return
                                } else {
                                    //textActions.toSelectMode(current_mode);
                                    //textActions.select(curtext, end.x * current_zoom, end.y * current_zoom);
                                    //textActions.setCursor(j);

                                    selblock = getElem("text_selectblock" + curConfig.backId);
                                    if (selblock) {
                                        var d = selblock.getAttribute('d');
                                        if (d) {
                                            //console.log(d);
                                            if (d == "M 0 0") {
                                                textActions.toSelectMode(current_mode);
                    ...
                    if (lenTspan > 0) {
                        ...
                        for (var j = 0; j < lenTspan; j++) {
                            ...
                            if (ptCurrent.y < end.y && ptCurrent.y >= topLimit && isTurn) {
                                ...
                                var isReSelect = current_mode !== 'textedit' || (j == lenTspan - 1);

                                if (isReSelect) textActions.select(curtext, end.x * current_zoom, end.y * current_zoom);

                                if (curtext.textContent.trim().length == 0) textActions.setCursor(0);
                                return;
                            }
                        }
                    } else {
        ...
        $(container).mousedown(mouseDown).mousemove(mouseMove).click(dblClick).mouseup(mouseUp);
    ...
    var textActions = canvas.textActions = function () {
        ...
        function setCursor(index) {
            var empty = (textinput.value === "" || textinput.value === '　');
            ...
            if (!cursor) {
                ...
                if (getElem("selectorParentGroup" + curConfig.backId)) {
                    cursor = getElem("selectorParentGroup" + curConfig.backId).appendChild(cursor);
                }
            }
        ...
        function setSelection(start, end, skipInput) {
            ...
            if (startbb && endbb) {
                ...
                if (textinput) {
                    if (start == textinput.selectionStart) {
                        selblock.endbb = endbb;
                    } else {
                        selblock.endbb = startbb;
                    }
                }
            }
        ...
        //            'style': "pointer-events:none"
        //        });
        //        getElem("selectorParentGroup" + curConfig.backId).appendChild(selblockRange);
        //        //console.log(getElem("selectorParentGroup" + curConfig.backId));
        //    }


        //    //if (index == 0) {
        //    //    console.log(chardataAll);
        ...
        //    if (startbb && endbb) {
        //        var fz = parseInt(window.getComputedStyle(curtextdataRange[index])['font-size']);
        ...
        function getIndexFromPoint(mouse_x, mouse_y) {
            ...
            var oldCurrent_mode = '';
            if (current_mode) {
                oldCurrent_mode = Ext.clone(current_mode);
            }

            //console.log(Ext.clone(current_mode));
            ...
            if (!mousePress) {
                getRangeBlurCharNumAtPosition();

                if (curtextdataRange && curtextdataRange.length > 0) {
                    Ext.Array.each(curtextdataRange, function (rangeText) {
                        var rangeCharpos = rangeText.getCharNumAtPosition(pt);
                        if (rangeCharpos >= 0) {
                            curtext = rangeText;
                            setChardataRange(curtext);

                            textActions.toEditChange();
                            textActions.start(rangeText)
            ...
            if (oldCurrent_mode && oldCurrent_mode != current_mode) {
                current_mode = oldCurrent_mode;
            }
        ...
        /*
        function getIndexFromPoint_End(mouse_x, mouse_y) {
        ...
        */
        ...
        function getRangeBlurCharNumAtPosition(pt) {
            chardataRange = [];
            curtextdataRange = [];
            //重置curtextdataRange,chardataRange 從最TOP那筆開始

            var current_layer = getCurrentDrawing().getCurrentLayer();
            var contexts = current_layer.querySelectorAll('text[id*="KDRichTextBlock"][id*="_context"]');
            for (var i = 0; i < contexts.length; i++) {
                curtextdataRange.push(contexts[i]);
                setChardataRange(contexts[i]);
            }

            /*
            var previousCurtext = null;
            if (curtext.previousSibling && curtext.previousSibling.previousSibling &&
                curtext.previousSibling.previousSibling.id &&
                curtext.previousSibling.previousSibling.id.indexOf('KDRichTextBlock') != -1 &&
                curtext.previousSibling.previousSibling.id.indexOf('context') != -1) {

                previousCurtext = curtext.previousSibling.previousSibling;
                doPrevious = true;

                do {
                    if (previousCurtext.previousSibling && previousCurtext.previousSibling.previousSibling &&
                        previousCurtext.previousSibling.previousSibling.id &&
                        previousCurtext.previousSibling.previousSibling.id.indexOf('KDRichTextBlock') != -1 &&
                        previousCurtext.previousSibling.previousSibling.id.indexOf('context') != -1) {

                        previousCurtext = previousCurtext.previousSibling.previousSibling;
            ...
            if (previousCurtext) curtext = previousCurtext;
            ...
            if (curtext && curtext.nextSibling) {
                ...
                do {
                    ...
                    if (nextCurtext.id.indexOf('KDRichTextBlock') == -1) {
                        doNext = false;
                    }
                    ...
                    if (doNext) {
                        ...
                        if (isPush) {
                            //check lost old Curtext
                            var doLost = false;
                            var doLostCurtext = curtext.nextSibling;
                            if (doLostCurtext && doLostCurtext != undefined) {
                                do {
                                    var lostPust = true;
                                    if (doLost) doLostCurtext = doLostCurtext.nextSibling;
                                    for (var j in curtextdataRange) {
                                        if (doLostCurtext.id == curtextdataRange[j].id) {
                                            lostPust = false;
                                        }
                                    }
                                    ...
                                    if (lostPust) {
                                        doLost = true;
                                        if (doLostCurtext.id !== nextCurtext.id) {
                                            if (doLostCurtext.id.indexOf('KDRichTextBlock') >= 0 &&
                                                doLostCurtext.id.indexOf('context') >= 0) {
                                                curtextdataRange.push(doLostCurtext);
                                                setChardataRange(doLostCurtext);
                                            }
                                        } else {
                            ...
                            if (doLostCurtext.id == 'KDRichTextBlock_4' && doLostCurtext.innerHTML == '辦法：') {
                                var visibility = doLostCurtext.getAttribute('visibility');
        ...
        function clearSelectionRange() {
            ...
            if (oldselblockRange) {
                getElem("selectorParentGroup" + curConfig.backId).removeChild(oldselblockRange);
                var selectorGroupFirst = getElem("selectorGroup0");
                if (selectorGroupFirst) {
                    getElem("selectorParentGroup" + curConfig.backId).removeChild(selectorGroupFirst);
                }    
            }
        ...
        return {
            ...
            mouseUp: function (evt, mouse_x, mouse_y) {
                ...
                if (selectedElements != null) {
                    if (selectedElements[0] && selectedElements[0].id) {
                        if (textinput) {
                            var selectionStart = Ext.clone(textinput.selectionStart);
                            var selectionEnd = Ext.clone(textinput.selectionEnd);
                            if (chardata && chardata.length > selectionStart) {
                                if (selectionStart == selectionEnd) {
                                    ...
                                    if (selblock) {
                                        ...
                                        if (d) {
                                            //console.log(d);
                                            if (d !== "M 0 0" && selblock.endbb) {
                                                pt.x = selblock.endbb.x;
                                                pt.y = selblock.endbb.y;
                                            }
                                        }
                                    }
                                }
            ...
            mouseDown: function (evt, mouse_target, start_x, start_y) {
                if (current_mode === "path") {
                    ...
                    if (!stretchy) {
                        ...
                        stretchy = getElem("selectorParentGroup" + curConfig.backId).appendChild(stretchy);
                    }
    ...
    this.svgCanvasToString = function () {
        ...
        $(svgcontent).find(document.getElementById('g:data(gsvg)')).each(function () {
    ...
    this.svgToString = function (elem, indent) {
        ...
        if (elem) {
            ...
            if (elem.id === 'svgcontent') {
                ...
                var i = attrs.length;
                var attr_names = ['width', 'height', 'xmlns', 'x', 'y', 'viewBox', 'id', 'overflow', 'text-decoration'];
                while (i--) {
                    attr = attrs.item(i);
    ...
    this.importSvgString = function (xmlString) {
        ...
        try {
            ...
            if (import_ids[uid]) {
                if ($(import_ids[uid].symbol).parents('#svgroot' + curConfig.backId).length) {
                    useExisting = true;
                }
            }
    ...
    var setContext = this.setContext = function (elem) {
        ...
        $(elem).parentsUntil('#svgcontent' + curConfig.backId).andSelf().siblings().each(function () {
    ...
    this.setResolution = function (x, y) {
        ...
        if (x != w || y != h) {
            ...
            var background = document.getElementById("canvas_background" + curConfig.backId);
            if (background) {
