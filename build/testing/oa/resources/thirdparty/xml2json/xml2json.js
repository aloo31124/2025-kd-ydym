            ...
            if (!parent                    // no parent = root element = first step in recursion
                ...
                if (xml.nodeType == 1) { // element node
                    if (xml.childNodes.length > 0) {
                        var tmp = X.toObj(xml, o);
                        if (tmp.文字) tmp = {tagName: "文字", childNodes: [tmp.文字]};
            ...
            if (xml.nodeType == 1) {   // element node ..
                if (xml.attributes.length)   // element with attributes  ..
                    for (var i = 0; i < xml.attributes.length; i++) {
                        parent[xml.attributes[i].nodeName] = xml.attributes[i].nodeValue;
                    ...
                    if (hasElementChild && !needsArray) { // Neatly structured and unique child elements, no plain text/cdata in the mix
                        ...
                        for (var n = xml.firstChild; n; n = n.nextSibling) {
                            ...
                            //o["#text"] = X.escape(n.nodeValue);
                                ...
                                var newObj = X.toObj(n, o);
                                newObj['tagName'] = n.nodeName;
