...
$.SvgCanvas = function (container, config) {
    ...
    svgedit.utilities.init(utilInitParas);
    if (curConfig.backId == "") window.utilInitParas = utilInitParas;
    ...
    var TextLineCommand = svgedit.history.TextLineCommand;
        ...
        handleHistoryEvent: function (eventType, cmd) {
            ...
            if (
                ...
                if (cmdType == MoveElementCommand.type()) {
                    ...
                    //	}
                    //}
                } else if (cmdType == TextLineCommand.type()) {
                    call("historyCommand", cmd);
                }
            }
        },
    });
    ...
    var getVisibleElements = (this.getVisibleElements = function (parent) {
        ...
        if (parent.find("#canvas_background" + curConfig.backId).length)
            parent.splice(0, 1); // Prevent background from being included
        var contentElems = [];
        $(parent)
            .children()
    ...
    this.prepareSvg = function (newDoc) {
        ...
        var docType = "";
        var el = newDoc.getElementById("docCaption_title");
        if (el != null) {
            docType = el.innerHTML;
            //console.log("docType:" + docType);
        }

        //2019.03.21 add "令" lien.chiu

        var elTitle = newDoc.getElementById("發文機關_title");
        var elSend = true;
        if (elTitle && elTitle.innerHTML.indexOf("令") !== -1) elSend = false;
        if (elTitle && !elSend) {
            if (elTitle.innerHTML.split("　").length > 1) {
                docType = elTitle.innerHTML.split("　")[1];
                if (docType.indexOf("（稿）") != -1) {
                    docType = docType.replace("（稿）", "");
                    //console.log(docType);
                }
            }
        }

        this.sanitizeSvg(newDoc.documentElement, docType);
        ...
        for (var i = 0, len = paths.length; i < len; ++i) {
            ...
            //TODO: pathSegList 最移除了，用到入徑要改 pathseg.js來進行修正
    ...
    var getMouseTarget = (this.getMouseTarget = function (evt) {
        ...
        if ($target.closest("#selectorParentGroup" + curConfig.backId).length) {
    ...
    (function () {
        ...
        var mouseDown = function (evt) {
            ...
            if (right_click) {
                //以外面右鍵為主，暫時關閉
                if (current_mode == "select" || current_mode == "textedit") return;
            ...
            switch (current_mode) {
                ...
                case "resize":
                    ...
                    init_bbox = svgedit.utilities.getBBox(
                        $("#selectedBox0" + curConfig.backId)[0]
                    );
                    var bb = {};
                ...
                case "rotate":
                    ...
                    document.getElementById("workarea" + curConfig.backId).className =
                        "rotate";
        ...
        var mouseUp = function (evt) {
            ...
            if (!keep && element != null) {
                ...
                if (
                    ...
                    t.parentNode.id != "selectorParentGroup" + curConfig.backId &&
                    t.id != "svgcanvas" + curConfig.backId &&
                    t.id != "svgroot" + curConfig.backId
                ) {
            ...
            //console.log('mouseUp ' + current_mode)
        };
        ...
        var dblClick = function (evt) {
            ...
            // console.log(tagName);
            // console.log(current_mode);
            // if (tagName === 'svg' && current_mode !== 'textedit') {
            if (tagName === "svg") {
                // return;
                var ptCurrent = transformPoint(evt.pageX, evt.pageY, root_sctm);
                // OA.common.Utils.testRect({name: 'ptCurrent', x: ptCurrent.x, y: ptCurrent.y});
                var current_layer = getCurrentDrawing().getCurrentLayer();
                var contexts = current_layer.querySelectorAll(
                    'text[id*="KDRichTextBlock"][id*="_context"]'
                );
                //var contexts = current_layer.querySelectorAll(
                //    'text[id*="KDRichTextBlock-"][id*="_context"],text[id*="KDRichTextBlock_"][id*="_context"]'
                //);              
                var first = svgedit.utilities.getBBox(contexts[0]);
                var topLimit = first.y;

                var len = contexts.length;
                for (var i = 0; i < len; i++) {
                    var curtext = contexts[i];
                    var chars = curtext.getNumberOfChars();
                    if (chars <= 0) continue;
                    //edit by job 2019/1/8 for edge bug
                    //var end = curtext.getEndPositionOfChar(chars - 1);
                    var end = svgedit.utilities.getEndPositionOfChar(curtext, chars - 1);
                    var tpans = curtext.children;
                    var lenTspan = tpans.length;
                    if (lenTspan > 0) {
                        var wordCount = 0;
                        for (var j = 0; j < lenTspan; j++) {
                            var tspan = tpans[j];

                            chars = tspan.getNumberOfChars();
                            //edit by job 2019/1/9 for edge bug
                            //end = tspan.getEndPositionOfChar(chars - 1);
                            end = svgedit.utilities.getEndPositionOfChar(tspan, chars - 1);
                            wordCount = wordCount + chars;

                            var fz = parseInt(window.getComputedStyle(tspan)["font-size"]);
                            // addSvgElementFromJson({
                            //     "element": "rect",
                            //     "curStyles": true,
                            //     "attr": {
                            ...
                            var isTurn = tpans[j + 1] && tpans[j + 1].hasAttribute("dy");
                            if (j + 1 == lenTspan) isTurn = true;
                            if (ptCurrent.y < end.y && ptCurrent.y >= topLimit && isTurn) {
                                // if (ptCurrent.y < end.y && isTurn) {
                                // var isSelect = true;
                                // var curs = svgroot.querySelectorAll('#' + 'text_cursor' + curConfig.backId);
                                // if (curs.length > 0 && current_mode == 'textedit' && curs[0].getAttribute('display') == 'inline' ) isSelect = false ;
                                var isReSelect =
                                    current_mode !== "textedit" || j == lenTspan - 1;
                                ...
                                if (curtext.textContent.trim().length == 0)
                                    textActions.setCursor(0);
                                return;
                            }
                        }
                    } else {
                        //var textbb = svgedit.utilities.getBBox(curtext);
                        ...
                        //     "attr": {
                        //         "x": end.x,
                        //         "y": end.y - textbb.height,
                        //         "width": 20,
                        //         "height": textbb.height
                        //     }
                        // });

                        if (ptCurrent.y < end.y && ptCurrent.y >= topLimit) {
                            textActions.select(curtext);
                            if (curtext.textContent.trim().length == 0)
                                textActions.setCursor(0);
                            return;
                        }
            ...
            if (tagName === "text" && current_mode !== "textedit") {
                var pt = transformPoint(evt.pageX, evt.pageY, root_sctm),
                    mouse_x = pt.x * (isBotchedZoom ? 1 : current_zoom),
                    mouse_y = pt.y * (isBotchedZoom ? 1 : current_zoom);
                textActions.select(mouse_target, mouse_x, mouse_y);

                //start_x *= current_zoom;
                //start_y *= current_zoom;
                //textActions.select(mouse_target, start_x, start_y);
            }
        ...
        var handleLinkInCanvas = function (evt) {
            ...
            //    var mouse_target = getMouseTarget(evt);
            //    var tagName = mouse_target.tagName;
            //    //console.log(evt.type + ' ' + current_mode + ' ' + tagName);
            //    if (parent === current_group) return;
            //    if (tagName === 'text' && current_mode !== 'textedit') {
            //        start_x *= current_zoom;
            //        start_y *= current_zoom;
        ...
        $(container).bind("mousewheel DOMMouseScroll", function (e) {
            //update by job 20190129 fix selection scroll bug
            if (current_mode == "textedit" && started) {
                root_sctmx = svgcontent.getScreenCTM().inverse();
                var ptx = transformPoint(e.pageX, e.pageY, root_sctmx);
                //console.log("scroll from " + e.pageX +":"+e.pageY+ " to " + ptx.x +":" + ptx.y);
                textActions.mouseScroll(ptx.x, ptx.y);
            }
    ...
    var textActions = (canvas.textActions = (function () {
        ...
        var chardata = [];
        var chardataRange = [];
        var curtextdataRange = [];
        var selectElementRange = {};
        var firstStart = 0;
        var textbb, transbb;
        var matrix;
        var last_x, last_y;
        var allow_dbl;
        //update by job 20190129 fix selection scroll bug
        var scroll_px, scroll_py;
        var mousePress = false;

        // 設定游標 By Xiang
        function setCursor(index) {
            var empty = textinput.value === "" || textinput.value === "　" || textinput.value === " ";

            if (current_mode == "textedit") $(textinput).focus();
            if (!arguments.length) {
                if (empty) {
                    index = 0;
            ...
            cursor = getElem("text_cursor" + curConfig.backId);

            if (!cursor) {
                cursor = document.createElementNS(svgns, "line");
                assignAttributes(cursor, {
                    id: "text_cursor" + curConfig.backId,
                    stroke: "#333",
                    //'stroke': "red",
                    "stroke-width": 1,
                });
                if (getElem("selectorParentGroup" + curConfig.backId)) {
                    cursor = getElem(
                        "selectorParentGroup" + curConfig.backId
                    ).appendChild(cursor);
                }
            }
            ...
            var fz = parseInt(window.getComputedStyle(curtext)["font-size"]);
            if (charbb) {
                var start_pt = ptToScreen(charbb.x, charbb.y - fz);
                var end_pt = ptToScreen(charbb.x, charbb.y);
                if (start_pt.y) {
                    var curs = svgroot.querySelectorAll(
                        "#" + "text_cursor" + curConfig.backId
                    );
                    assignAttributes(cursor, {
                        x1: start_pt.x,
                        y1: start_pt.y,
        ...
        function setIMECursor(index) {
            if (!index) index = textinput.selectionEnd;
            ...
            if (!cursor) {
                ...
                cursor = getElem("selectorParentGroup" + curConfig.backId).appendChild(
                    cursor
                );
            }
         ...
         * 設定選取功能 By Xiang
         */
        const setSelection = async (start, end, skipInput) => {
            await curtextdataRangeSort();
            await setSelection1(start, end, skipInput);
            await multipleSelection(start, end, skipInput);
        }
         ...
         * @param {*} skipInput
         */
        const setSelection1 = async (start, end, skipInput) => {
            return new Promise((resolve) => {
                ...
                selblock = getElem("text_selectblock" + curConfig.backId);
                if (!selblock) {
                    selblock = document.createElementNS(svgns, "path");
                    assignAttributes(selblock, {
                        id: "text_selectblock" + curConfig.backId,
                        fill: "green",
                        opacity: 0.5,
                        style: "pointer-events:none",
                    });
                    getElem("selectorParentGroup" + curConfig.backId).appendChild(selblock);
                }

                // 設定第一行開始位置 By Xiang
                if (chardataRange.length == 0) {
                    firstStart = end;
                }
        ...
        const multipleSelection = async (start, end, skipInput, rangeText) => {
            return new Promise((resolve) => {
                let startbb = chardata[start];
                let endbb = chardata[end];
                let rangeText = ""
                let selectCharDataRange = {}

                /**表示有選取第一行文字
                * 設定 firstStart 為第一行開始字元位置（必須-1）By Xiang
                * startbb 抓取選取的最後一個元素
                * endbb 抓取該行最後一個元素
                */
                if (chardataRange.length == 1) {
                    startbb = chardataRange[0][end];
                    endbb = chardataRange[0][chardataRange[0].length - 1];
                    firstStart = textinput.selectionStart;
                }

                // 表示選取跨行 By Xiang
                if (chardataRange.length > 1) {
                    Ext.Array.each(chardataRange, function (range, index) {
                        if (mouseState === 1) {
                            // 抓取目前使用者選擇的最後一行 By Xiang
                            if (index == chardataRange.length - 1) {
                                // 取得該行第一個字元 By Xiang
                                startbb = range[0];
                                // 取得該行最後一個字元 By Xiang
                                endbb = range[end];
                                if (endbb == undefined) {
                                    if (start == -1) {
                                        ...
                                        endbb = range[start];
                                        curtext = curtextdataRange[index];
                                        rangeText =
                                            rangeText +
                                            curtextdataRange[index].textContent.substr(0, start);
                                        selectCharDataRange[index] = curtextdataRange[index].textContent.substr(0, start);
                                        selectElementRange[index] = [0, start, range.length, curtextdataRange[index].id.replace("_context", "")]
                                    }
                                } else {
                                    // 重新設定元素至滑鼠到達的最新元素 By Xiang
                                    curtext = curtextdataRange[index];
                                    rangeText = rangeText + curtextdataRange[index].textContent.substr(0, end);
                                    selectCharDataRange[index] = curtextdataRange[index].textContent.substr(0, end);
                                    selectElementRange[index] = [0, end, range.length, curtextdataRange[index].id.replace("_context", "")]
                                }
                            } else {
                                // 若不屬於最新的一行，則將該行選取範圍傳入 setRangeSelection By Xiang
                                setRangeSelection(range, index, start);

                                // 第一行必須挑選開始文字位置 By Xiang
                                if (index == 0) {
                                    rangeText = curtextdataRange[index].textContent.substr(
                                        firstStart,
                                        range.length
                                    );
                                    selectCharDataRange[index] = curtextdataRange[index].textContent.substr(
                                        firstStart,
                                        range.length
                                    );
                                    selectElementRange[index] = [firstStart, range.length, range.length, curtextdataRange[index].id.replace("_context", "")]
                                } else {
                                    rangeText = rangeText + curtextdataRange[index].textContent;
                                    selectCharDataRange[index] = curtextdataRange[index].textContent;
                                    selectElementRange[index] = [0, range.length, range.length, curtextdataRange[index].id.replace("_context", "")]
                                }
                            }
                        }

                        if (mouseState === 2) {
                            // console.log(chardataRange);
                            if (index == 0) {
                                // 取得該行第一個字元 By Xiang
                                startbb = range[start];
                                // 取得該行最後一個字元 By Xiang
                                ...
                                if (endbb == undefined) {
                                    ...
                                    // 重新設定元素至滑鼠到達的最新元素 By Xiang
                                    curtext = curtextdataRange[index];
                                    rangeText = rangeText + curtextdataRange[index].textContent.substr(start, end);
                                    selectElementRange[index] = [start, range.length, range.length, curtextdataRange[index].id.replace("_context", "")]
                                }
                            } else {
                                // 若不屬於最新的一行，則將該行選取範圍傳入 setRangeSelection By Xiang
                                setRangeSelection(range, index, start);
                    ...
                    // 執行事件 By Xiang
                    $(document).bind("keydown", "67", function (event) {
                        if (event.keyCode === 67 && event.ctrlKey && chardataRange.length > 0 && selectElementRange) {
                            //要重新計算rangeText
                            if (curtext) {
                                console.log(curtextdataRange[chardataRange.length - 1]);
                                console.log(curtext);
                                //id不同表示最後一行沒有被選取
                                if (curtext.id !== curtextdataRange[curtextdataRange.length - 1].id &&
                                    chardataRange.length == curtextdataRange.length) {
                                    rangeText = '';
                                    console.log('remove last curtextdataRange');
                                    curtextdataRange.pop();
                                    //chardataRange.shift();
                                    for (let i = 0; i < curtextdataRange.length; i++) {

                                        for (let k = 0; k < curtextdataRange.length; k++) {
                                            if (selectElementRange[k] &&
                                                selectElementRange[k][3] == curtextdataRange[i].id.replace('_context', '')) {
                                                rangeText = rangeText + curtextdataRange[i].textContent.substr(selectElementRange[k][0], selectElementRange[k][1])
                                            }
                                        }

                                    }
                                } else if (curtext.id == curtextdataRange[curtextdataRange.length - 1].id &&
                                    chardataRange.length == curtextdataRange.length) {
                                    rangeText = '';
                                    for (let i = 0; i < curtextdataRange.length; i++) {
                                        for (let k = 0; k < curtextdataRange.length; k++) {
                    ...
                    //remove old text_selectblockRange
                    oldselblockRange = getElem("text_selectblockRange0");
                    if (oldselblockRange)
                        getElem("selectorParentGroup" + curConfig.backId).removeChild(
                            oldselblockRange
                        );

                    var oldindex = 1;
                    do {
                        oldselblockRange = getElem("text_selectblockRange" + oldindex);
                        if (oldselblockRange)
                            getElem("selectorParentGroup" + curConfig.backId).removeChild(
                                oldselblockRange
                            );
                        oldindex++;
                    } while (oldselblockRange);
                }

                if (cursor) cursor.setAttribute("visibility", "hidden");

                //跨行選取出現螢光棒 By Xiang
                if (startbb && endbb && curtext) {
                    var fz = parseInt(window.getComputedStyle(curtext)["font-size"]);
                    var curbb = curtext.getBBox();
                    var tl = ptToScreen(startbb.x, startbb.y - fz + 2),
                        tr = ptToScreen(curbb.x + curbb.width, startbb.y - fz + 2),
                        trm = ptToScreen(curbb.x + curbb.width, endbb.y - fz + 2),
                        brm = ptToScreen(startbb.x + (endbb.x - startbb.x), endbb.y - fz + 2),
                        br = ptToScreen(startbb.x + (endbb.x - startbb.x), endbb.y + 4),
                        bl = ptToScreen(curbb.x, endbb.y + 4),
                        blm = ptToScreen(curbb.x, startbb.y + 4),
                        tlm = ptToScreen(startbb.x, startbb.y + 4);
                ...
                call("selection", skipInput);
                resolve();
            })
        }

        const curtextdataRangeSort = () => {
            return new Promise((resolve) => {
                if (curtextdataRange.length === 0) {
                    resolve();
                }
                const regex = /([0])+\d+(?=_context)/;
                for (let i = 0; i < curtextdataRange.length; i++) {
                    let itemData = curtextdataRange[i];
                    let minIndex = i;
                    for (let j = i; j < curtextdataRange.length; j++) {
                        if (curtextdataRange[i].id.match(regex) && curtextdataRange[j].id.match(regex) &&
                            parseInt(curtextdataRange[i].id.match(regex)[0]) > parseInt(curtextdataRange[j].id.match(regex)[0])
                        ) {
                            itemData = curtextdataRange[j];
                            minIndex = j;
                        }
                    }
                    [chardataRange[minIndex], chardataRange[i]] = [chardataRange[i], chardataRange[minIndex]];
                    [curtextdataRange[minIndex], curtextdataRange[i]] = [curtextdataRange[i], curtextdataRange[minIndex]];
                    if (i == curtextdataRange.length - 1) {
                        resolve();
                    }
                }
            });
        ...
        function setRangeSelection(chardataAll, index, start) {
            selblockRange = getElem("text_selectblockRange" + index);

            // 跨行選取時更換顏色 By Xiang
            if (!selblockRange) {
                selblockRange = document.createElementNS(svgns, "path");
                assignAttributes(selblockRange, {
                    id: "text_selectblockRange" + index,
                    fill: "grey",
                    opacity: 0.5,
                    style: "pointer-events:none",
                });
                getElem("selectorParentGroup" + curConfig.backId).appendChild(
                    selblockRange
                );
            }
            ...
            // 當滑鼠往下選取時 By Xiang
            if (mouseState === 1) {
                var startbb = index == 0 ? chardataAll[firstStart] : chardataAll[0];
                var endbb = chardataAll[chardataAll.length - 1];
            }

            // 當滑鼠往上選取時 By Xiang
            if (mouseState === 2) {
                var startbb = index == 0 ? chardataAll[firstStart] : chardataAll[0];
                var endbb = (index == curtextdataRange.length - 1) ? chardataAll[firstStart] : chardataAll[chardataAll.length - 1];
            }

            if (startbb && endbb) {
                var fz = parseInt(
                    window.getComputedStyle(curtextdataRange[index])["font-size"]
                );
                var curbb = curtextdataRange[index].getBBox();
                var tl = ptToScreen(startbb.x, startbb.y - fz + 2),
                    tr = ptToScreen(curbb.x + curbb.width, startbb.y - fz + 2),
                    trm = ptToScreen(curbb.x + curbb.width, endbb.y - fz + 2),
                    brm = ptToScreen(startbb.x + (endbb.x - startbb.x), endbb.y - fz + 2),
        ...
        function getBlurCharNumAtPosition(pt) {
            ...
            if (charpos < 0) {
                ...
                if (chardata[0]) {
                    if (pt.x <= chardata[0].x) {
                        charpos = 0;
                    }
                }
                //revise point not select
                pt.x = pt.x + 4;
                pt.y = pt.y;
                charpos = curtext.getCharNumAtPosition(pt);
        ...
        function getIndexFromPoint(mouse_x, mouse_y) {
            ...
            var charpos = getBlurCharNumAtPosition(pt);
            if (charpos >= chardata.length - 2) {
                charpos = chardata.length - 2;
            }
            if (charpos < 0) return charpos;
        ...
        function getRangeIndexFromPoint(mouse_x, mouse_y) {
            var pt = svgroot.createSVGPoint();
            pt.x = mouse_x;
            pt.y = mouse_y;

            // 判斷是否有讀取到該行每個字元資料（x, y）
            if (chardata.length == 1) return 0;

            // 判斷 text 元素
            if (!curtext) return 0;

            var charpos = getRangeBlurCharNumAtPosition(pt);
            if (charpos >= chardata.length - 2) {
                charpos = chardata.length - 2;
            }
        ...
        function getRangeBlurCharNumAtPosition(pt) {
            //抓全部的段落來判斷
            // 判斷滑鼠目前所在的 pointer 在元素curtext的哪一個位置，若不存在會回傳 -1
            var charpos = curtext.getCharNumAtPosition(pt);
            if (charpos < 0) {
                // Out of text range, look at mouse coords
                //charpos = chardata.length - 2;

                // chardata 存放該行所有字元元素資料 By Xiang
                if (chardata[0]) {
                    if (pt.x <= chardata[0].x) {
                        charpos = 0;
                    }
                ...
                if (
                    ...
                    var numdata = [];
                    for (var num in chardata) {
                        if (pt.y >= chardata[num].y) {
                            if (numdata.length > 0) {
                                if (numdata[0].y <= chardata[num].y) {
                                    numdata.push(chardata[num]);
                                }
                            } else {
                                numdata.push(chardata[num]);
                            }
                    ...
                    if (numdata.length > 0) {
                        //抓最後一筆
                        var last = numdata[numdata.length - 1];
                        var outPt = svgroot.createSVGPoint();
                        outPt.x = last.x;
                        outPt.y = last.y;
                        charpos = curtext.getCharNumAtPosition(outPt);
                    }
                }
                ...
                if (mousePress) {
                    // 向上選取
                    /*
                    if (pt.y < curtext.getBBox().y - 3) {
                        let previousElement;
                        let testContexts = svgcontent.querySelectorAll(
                            'text[id*="KDRichTextBlock-"][id*="_context"],text[id*="KDRichTextBlock_"][id*="_context"]'
                        )
                        index = [].indexOf.call(testContexts, curtext);
                        if (index !== 0) {
                            previousElement = testContexts[index - 1];
                            nextCharpos = previousElement.getCharNumAtPosition(pt);

                            pt.x = pt.x + 4;
                            pt.y = pt.y;
                            var nextCharpos = previousElement.getCharNumAtPosition(pt);
                            if (nextCharpos < 0) {
                                pt.x = pt.x;
                                pt.y = pt.y + 10;
                                nextCharpos = previousElement.getCharNumAtPosition(pt);
                            ...
                            if (nextCharpos > 0) {
                                // TODO: Xiang 判斷向上元素
                                if (curtextdataRange.length == 0) {
                                    curtextdataRange.push(previousElement);
                                    setTopCharDataRange(previousElement);
                                    curtextdataRange.push(Ext.clone(curtext));
                                    setTopCharDataRange(Ext.clone(curtext));
                                } else {
                                    var isPush = true;
                                    Ext.Array.each(curtextdataRange, function (item) {
                                        if (item.id === previousElement.id) {
                                            isPush = false;
                                            return false;
                                        }
                                    });
                    ...
                    */
                    ...
                    // 向下選取
                    if (pt.y > curtext.getBBox().y + 40) {
                        mouseState = 1
                        var nextCurtext = curtext.nextSibling;
                        var doNext = true;
                        do {
                            if (doNext) nextCurtext = nextCurtext.nextSibling;
                            if (!nextCurtext) return charpos;
                            if (nextCurtext.id.indexOf("gSeal") != -1) {
                                doNext = false;
                                return charpos;
                            }
                            ...
                            pt.x = pt.x + 4;
                            pt.y = pt.y;
                            var nextCharpos = nextCurtext.getCharNumAtPosition(pt);
                            if (nextCharpos < 0) {
                                pt.x = pt.x;
                                pt.y = pt.y - 10;
                                nextCharpos = nextCurtext.getCharNumAtPosition(pt);
                            }

                            if (nextCharpos > 0) {
                                //加入chardataRange
                                if (curtextdataRange.length == 0) {
                                    curtextdataRange.push(curtext);
                                    setChardataRange(curtext);
                                } else {
                                    var isPush = true;
                                    // 檢查 curtextdataRange 是否有此行元素資料
                                    Ext.Array.each(curtextdataRange, function (item) {
                                        if (item.id == nextCurtext.id) {
                                            isPush = false;
                                            return false;
                                        }
                                    });
                                    if (isPush) {
                                        curtextdataRange.push(nextCurtext);
                                        setChardataRange(nextCurtext);
                                    }
                                }
                                charpos = nextCharpos;
                            }
        ...
        function setChardataRange(nextcurtext) {
            var str = nextcurtext.textContent;
            var len = str.length;

            var xform = curtext.getAttribute("transform");

            var textbbRange = svgedit.utilities.getBBox(nextcurtext);

            matrix = xform ? getMatrix(nextcurtext) : null;
            ...
            // var tpans = curtext.childNodes;
            var tpans = nextcurtext.children;
            var tspanlast = [];
            if (tpans.length > 0) {
                var wordCount = 0;
                for (var idx = 0; idx < tpans.length; idx++) {
                    var tspan = tpans[idx];
                    wordCount = wordCount + tspan.getNumberOfChars();
                    var isTurn = tpans[idx + 1] && tpans[idx + 1].hasAttribute("dy");
                    if (idx + 1 == tpans.length) isTurn = true;
                    if (isTurn) tspanlast.push(wordCount);
                }
            }

            if (!len) {
                var end = { x: textbbRange.x + textbbRange.width / 2, width: 0 };
            }
            for (var i = 0; i < len; i++) {
                if (nextcurtext.getNumberOfChars() <= 0) continue;

                //edit by job 2019/1/9 edge bug
                //var start = curtext.getStartPositionOfChar(i);
                //var end = curtext.getEndPositionOfChar(i);
                var start = svgedit.utilities.getStartPositionOfChar(nextcurtext, i);
                var end = svgedit.utilities.getEndPositionOfChar(nextcurtext, i);
                ...
                var _rowIdx = tpans.length - 1;
                // TODO Decide if y, width and height are actually necessary
                chardata[i] = {
                    x: start.x,
                    y: start.y,
                    width: end.x - start.x,
                    height: textbbRange.height,
                    rowIdx: _rowIdx,
                };

                //tspan last
                var islast = tspanlast.length > 0 && tspanlast.indexOf(i) >= 0;
                if (islast) {
                    chardata[i] = {
                        x: chardata[i - 1].x + chardata[i - 1].width,
                        y: chardata[i - 1].y,
                        width: end.x - start.x,
                        height: textbbRange.height,
                        rowIdx: _rowIdx,
            ...
            chardataRange.push(chardata);
        }

        function setTopCharDataRange(nextcurtext) {
            // 文字內容
            var str = nextcurtext.textContent;
            // 文字內容長度
            var len = str.length;
            ...
            // 向上時需要塞入每行最後一個元素 By Xiang
            if (end) {
                chardata.push({
                    x: chardata[len - 1].x + chardata[len - 1].width,
                    y: end.y,
                    width: 0,
                });
            }
        ...
        function clearSelectionRange() {
            ...
            curtextdataRange = [];
            var $text = $("#text");
            $text.selectRange = undefined;
            var oldindex = 1;

            var oldselblockRange = getElem("text_selectblockRange0");
            if (oldselblockRange) {
                getElem("selectorParentGroup" + curConfig.backId).removeChild(
                    oldselblockRange
                );
                // 無效
                var selectorGroupFirst = getElem("selectorGroup0");
                if (selectorGroupFirst) {
                    getElem("selectorParentGroup" + curConfig.backId).removeChild(
                        selectorGroupFirst
                    );
                }
        ...
        function setCursorFromPoint(mouse_x, mouse_y) {
            var pos = getIndexFromPoint(mouse_x, mouse_y);
            if (pos < 0) return;
            setCursor(pos);
        }
        ...
        function setEndSelectionFromPoint(x, y, apply) {
            ...
            // 取得滑鼠最後選取字元（1為最左字元）
            var i2 = getRangeIndexFromPoint(x, y);
            var start = Math.min(i1, i2);
            var end = Math.max(i1, i2);
            if (curtextdataRange.length > 1 && mouseState === 1) {
                start = 0;
                end = i2;
            }

            if (curtextdataRange.length > 1 && mouseState === 2) {
                // 取得上面的元素長度
                let testContexts = svgcontent.querySelectorAll(
                    'text[id*="KDRichTextBlock-"][id*="_context"],text[id*="KDRichTextBlock_"][id*="_context"]'
                )
                let index = [].indexOf.call(testContexts, curtext);
                start = i2;
                // 取得新的一行最後一個字元位置
                end = testContexts[index].children[0].textContent.length;
            }
        ...
        return {
            ...
            mouseDown: function (evt, mouse_target, start_x, start_y) {
                ...
                //    //先找y
                //    Ext.Array.each(chardata, function (char1) {
                //        console.log(char1.y)
                //        if (char1.y > pt.y) {
                //            //如果musterchar已經有第一筆了要以第一筆為基準
                //            if (musterchar.length != 0) {
                //                if (musterchar[0].y == char1.y) {
                //                    musterchar.push(char1);
                //                }
                //            } else {
                //                musterchar.push(char1);
                //            }
                ...
                //    })
                //    if (musterchar.length == 0) {
                //        var lastchar = chardata[chardata.length - 1];
                //        Ext.Array.each(chardata, function (char2) {
                //            if (char2.y == lastchar.y) {
                //                musterchar.push(char2);
                //            }
                //        });
                //    }

                //    console.log(musterchar);

                //    //再找x
                //    var definechar = [];
                //    Ext.Array.each(musterchar, function (char3, index) {
                //        if (char3.x > pt.x) {
                //            definechar.push(Ext.clone(musterchar[index]));
                //            definechar[0].x = pt.x;
                //            return false;
                //        }
                //    })

                //    if (definechar.length == 0) {
                //        definechar.push(musterchar[musterchar.length - 1]);
                //    }
            ...
            //update by job 20190129 fix selection scroll bug
            mouseScroll: function (sx, sy) {
                //console.log("sx:sy"+ sx +":" + sy);
                setEndSelectionFromPoint(sx, sy);
                scroll_px = sx;
                scroll_py = sy;
            },
            mouseUp: function (evt, mouse_x, mouse_y) {
                ...
                var bb1 = ptToScreen(textbb.x, textbb.y);
                var bb2 = ptToScreen(textbb.x + textbb.width, textbb.y + textbb.height);
                ...
                //update by job 20190129 fix selection scroll bug
                if (scroll_py) {
                    setEndSelectionFromPoint(pt.x, scroll_py, true);
                } else {
                ...
                if (
                    ...
                    if (chardataRange.length == 0) {
                        textActions.toEditChange();
                        call("texteditMouseUp", getMouseTarget(evt));
                    }
                }
            ...
            init: function (inputElem) {
                ...
                $(curtext).unbind("dblclick", selectWord);
                ...
                for (var i = 0; i < len; i++) {
                    if (curtext.getNumberOfChars() <= 0) continue;

                    //edit by job 2019/1/9 edge bug
                    //var start = curtext.getStartPositionOfChar(i);
                    //var end = curtext.getEndPositionOfChar(i);
                    var start = svgedit.utilities.getStartPositionOfChar(curtext, i);
                    var end = svgedit.utilities.getEndPositionOfChar(curtext, i);
                ...
                if (end) {
                    ...
                    //調整跨行選取
                    //if (islast) {
                    //    setSelection(textinput.selectionStart, len, true);
                    //} else {
            ...
            getSelectElementRange: function () {
                return selectElementRange
            },
            screenToPt: function (x_in, y_in) {
                return screenToPt(x_in, y_in);
            },
        };
    })());
    ...
    this.svgCanvasToString = function () {
        ...
        $("#canvasGrid" + curConfig.backId).attr("display", "none");
        ...
        $(svgcontent)
            .find(document.getElementById("g:data(gsvg)"))
            .each(function () {
        ...
        if (elem) {
            ...
            if (elem.hasChildNodes()) {
                ...
                for (var i = 0; i < childs.length; i++) {
                    ...
                    switch (child.nodeType) {
                        case 1: // element node
                            //between tspan render to have space
                            if (child.tagName == "tspan") {
                                out.push("<!--\n-->");
                                out.push(this.svgToString(childs.item(i), 0));
                            } else {
                                out.push("\n");
    ...
    this.setSvgString = function (xmlString) {
        ...
        //console.log(content.find('image'));
        ...
        var background = $("#canvas_background" + curConfig.backId);
        ...
        //    methodDraw.paintBox.canvas.setPaint(fill)   why????
        canvas.setPaint(fill);
    ...
    this.importSvgString = function (xmlString) {
        try {
            ...
            if (import_ids[uid]) {
                if (
                    $(import_ids[uid].symbol).parents("#svgroot" + curConfig.backId)
                        .length
                ) {
                    useExisting = true;
                }
    ...
    var setContext = (this.setContext = function (elem) {
        ...
        $(elem)
            .parentsUntil("#svgcontent" + curConfig.backId)
            .andSelf()
            .siblings()
            .each(function () {
    ...
    this.setResolution = function (x, y) {
        ...
        if (x != w || y != h) {
            ...
            var background = document.getElementById(
                "canvas_background" + curConfig.backId
            );
            if (background) {
    ...
    this.setMode = function (name) {
        ...
        $("#" + "workarea" + curConfig.backId).attr("class", name);
        cur_properties =
    ...
    this.svgrootUtilInit = function () {
        if (svgroot.id !== "svgroot") svgedit.utilities.init(window.utilInitParas);
    };
        ...
        while (i--) {
            ...
            if (oldval !== String(newValue)) {
                if (attr == "#text") {
                    ...
                    var hasAttr = elem.hasAttribute(attr);
                    if (hasAttr) elem.setAttribute(attr, newValue);
                }
    ...
    this.updateCanvas = function (w, h) {
        ...
        var bg = $("#canvasBackground" + curConfig.backId)[0];
    ...
    this.setBackground = function (color, url) {
        var bg = getElem("canvasBackground" + curConfig.backId);
    ...
    var canDo = function (evt) {
        if (current_mode === "touch") {
            evt.preventDefault();
            return false;
        }

        if (current_mode == "fhpath") return true;
        var id = evt.target.getAttribute("id");
        var cls = evt.target.getAttribute("class");
        if (evt.target.tagName == "tspan") {
            id = evt.target.parentNode.getAttribute("id");
            cls = evt.target.parentNode.getAttribute("class");

            if (evt.type === "mousedown") call("tspanMousedown", evt.target);
        }
        if (!id) return false;

        var isKDRich = id.indexOf("KDRichTextBlock") >= 0;
        if (isKDRich) {
            if (cls.indexOf("indent") >= 0) {
                if (evt.type !== "mousedown") call("fieldSelected", evt.target);
                evt.preventDefault();
                return false;
            }
        } else {
            if (id == "svgroot" + curConfig.backId) {
                clearSelection(true);
                return true;
            } else {
                if (evt.type !== "mousedown") {
                    call("fieldSelected", evt.target);
                } else {
                    call("texteditMouseUp", evt.target);
                    return true;
                }
                return false;
            }
    ...
    var canMove = function () {
        if (current_mode == "select" || current_mode == "multiselect") return false;
        else return true;
    };
};
