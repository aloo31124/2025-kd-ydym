...
// Returns array with element associated with this command
    ...
    // 換行復元
    svgedit.history.TextLineCommand = function (options) {
        Ext.apply(this,options);
    };
    svgedit.history.TextLineCommand.type = function () {
        return 'svgedit.history.TextLineCommand';
    };
    svgedit.history.TextLineCommand.prototype.type = svgedit.history.TextLineCommand.type;
    svgedit.history.TextLineCommand.prototype.getText = function () {
        return this.text;
    };
    svgedit.history.TextLineCommand.prototype.apply = function (handler) {
        if (handler) {
            handler.handleHistoryEvent(svgedit.history.HistoryEventTypes.BEFORE_APPLY, this);
        }
       this['do']='redo';
    ...
    svgedit.history.TextLineCommand.prototype.unapply = function (handler) {
        if (handler) {
            handler.handleHistoryEvent(svgedit.history.HistoryEventTypes.BEFORE_UNAPPLY, this);
        }
        this['do']='undo';
    ...
    svgedit.history.TextLineCommand.prototype.elements = function () {
        return [this.elem];
    };
...
// Performs an undo step
    svgedit.history.UndoManager.prototype.undo = function () {
        if (this.undoStackPointer > 0) {
            ...
            //undo 做input 原paper.js undo拉到這邊處理 lien.chiu 2019.03.15
            if (cmd && cmd.text && cmd.text == '加字') {
...
// Performs a redo step
    svgedit.history.UndoManager.prototype.redo = function () {
        if (this.undoStackPointer < this.undoStack.length && this.undoStack.length > 0) {
            var cmd = this.undoStack[this.undoStackPointer++];
            //redo 做input 原paper.js redo拉到這邊處理 lien.chiu 2019.03.15
            if (cmd && cmd.text && cmd.text == '加字') {
                return;
            }
